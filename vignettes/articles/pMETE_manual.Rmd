...
```{r initiate format, include=FALSE, eval = TRUE}
library(formatR)
library(here)
i_am("vignettes/articles/pMETE_manual.Rmd")
...
load(here("vignettes/articles/size_data_com.rda"))
...
All fishes within a single sampling event ("ope_id") are defined as a fish community. Individual fish and fish community are filtered based on their body length ($\ge 60$ mm, since fish below this cut-off is underrepresented) and sample size ($\ge 50$ individuals, which was tested to be the smallest sample size necessary for numerical convergence of the pMETE model, see [@xu2023b]). 

```{r setup code ,message=FALSE, eval = FALSE, asis = TRUE}
library(zipfR) # for calculating incomplete gamma function for quasi-Weibull likelihood
library(tidyverse) # for data analysis and plotting
library(nleqslv) # for solving nonlinear equation

data("size_data_com") # individual size data from a single fish community
...
com_summary <- test_data %>%
  ...
  ungroup() %>% 
  # this line finds lambda_2 for the original METE
  mutate(L2_com_mete=S_0/(E_0_mete-N_0)) 
...
negLL_pop_pmete = function(par,x,xmin,n,L2,bc,m_com){
  # population-level metabolic scaling exponent
  bp = par[1]  
  # exact likelihood function from eqn 7.25 in Harte (2011)
  negLL_exact = -n*log(bp*L2*n)+(1-bp)*sum(log(x/xmin))+L2*n*sum((x/xmin)^bp)+n*log(exp(-n*L2)-exp(-n*L2*sum((m_com/min(m_com))^bc)))  
  return(negLL_exact)
}
...
negLL_qwei = function(par,x,xmin,n){
  # quasi-Weibull distribution parameter alpha
  qal = par[1]  
  # quasi-Weibull distribution parameter beta
  qbe = par[2]  
  # quasi-Weibull distribution parameter gamma
  qga = par[3]  
...
pop_stat_allmod <- full_join(pop_stat,pop_stat_othermod,by="ope_id") 
...
pop_stat_allmod_aicc <- pop_stat_allmod %>%
  ...
  # extract aicc columns and models
  dplyr::select(-mod) 
...
pop_stat_selectmod_aicc <- pop_stat_allmod_aicc %>% 
  ...
  filter(aicc_diff<=2) %>%
  # choose the model with a delta aicc <=2
  ungroup() 

# selected model for this brown trout population
pop_stat_selectmod_aicc 
```
